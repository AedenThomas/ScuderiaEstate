...
export const fetchPropertyDataByPostcode = async (postcode) => {
  ...
  const proxyUrl = `${proxyBaseUrl}/api/land-registry?postcode=${encodeURIComponent(
    cleanedPostcode
  )}`;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const errorMessage =
        data?.error || `Proxy error: ${response.status} ${response.statusText}`;
      console.error(`Proxy fetch failed: ${response.status}`, data);
      throw new Error(errorMessage);
    }

    console.log(
      `Proxy API Response items count: ${data?.result?.items?.length ?? 0}`
    );
...
export const formatTransactionData = (apiData) => {
  // Check for the actual items array
  if (!apiData || !apiData.result || !apiData.result.items) {
    // Simplified check
    if (apiData?.error) {
      console.warn(
        `formatTransactionData received an error object from proxy: ${apiData.error}`
      );
      return [];
    }
    console.log(
      "formatTransactionData: No transaction items found in API data structure."
    );
    return [];
  }
  ...
  // ADD LOGGING HERE:
  console.log(
    `formatTransactionData: Found ${apiData.result.items.length} raw items from API.`
  );

  if (apiData.result.items.length === 0) {
    return []; // Return early if API explicitly gave 0 items
  }

  const transactions = apiData.result.items
    .map((item) => {
      ...
      try {
        ...
        if (isNaN(transactionDate.getTime())) {
          // console.warn(`Invalid date format: ${item.transactionDate}`); // Less verbose
          transactionDate = null;
        }
      } catch (e) {
        console.error(`Error parsing date: ${item.transactionDate}`, e);
        transactionDate = null;
      }
      const paon = item.propertyAddress?.paon || "";
      const saon = item.propertyAddress?.saon || "";
      const street = item.propertyAddress?.street || "";
      const addressLine = `${saon} ${paon} ${street}`
        .replace(/\s+/g, " ")
        .trim();
      ...
      return {
        ...
        propertyType:
          item.propertyType?.prefLabel?.[0]?._value || "Unknown Type",
        isNewBuild: item.newBuild === true,
      };
    })
    .filter(
      (t) => t.date !== null && typeof t.price === "number" && !isNaN(t.price)
    ); // Keep filter

  console.log(`Formatted ${transactions.length} valid transactions.`);
  return transactions.sort((a, b) => b.date.getTime() - a.date.getTime()); // Keep sort
};
...
export const calculatePriceGrowth = (transactions) => {
  ...
  if (
    isNaN(firstTransaction.price) ||
    isNaN(latestTransaction.price) ||
    firstTransaction.price <= 0
  ) {
    console.warn("Price Growth: Invalid price data for calculation.");
    return {
      priceRange: { min: minPrice, max: maxPrice },
      growth: "Calculation error",
      annualizedReturn: "Calculation error",
    };
  }
  ...
  const timeDiffMs =
    latestTransaction.date.getTime() - firstTransaction.date.getTime();
  const yearsDiff =
    timeDiffMs > 0 ? timeDiffMs / (1000 * 60 * 60 * 24 * 365.25) : 0;
  ...
  if (yearsDiff > 0) {
    ...
    if (!isNaN(annualizedReturn)) {
      annualizedReturnFormatted = `${annualizedReturn.toFixed(1)}% p.a.`;
    } else {
      annualizedReturnFormatted = "Calculation error";
    }
  } else if (yearsDiff === 0 && percentageChange !== 0) {
    annualizedReturnFormatted = "Significant change within same year";
  } else if (yearsDiff === 0 && percentageChange === 0) {
    annualizedReturnFormatted = "0.0% p.a.";
  }

  console.log(
    `Price Growth Calculated: Growth=${percentageChange.toFixed(
      1
    )}%, Annualized=${annualizedReturnFormatted}`
  );
  return {
    priceRange: { min: minPrice, max: maxPrice },
    growth: `${percentageChange > 0 ? "+" : ""}${percentageChange.toFixed(
      1
    )}% over ${yearsDiff.toFixed(1)} years`,
